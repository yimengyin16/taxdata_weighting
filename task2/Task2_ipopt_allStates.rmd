...
state_names     <- as.character(acs_subset$stabbr) %>% unique
...
wgt_factor <- 1 # unnecessary, to be consistent with with function for single state


data_use <-     
  data %>% 
  rename(pwgtp_original = pwgtp) %>% 
  mutate(pwgtp = pwgtp_original * wgt_factor,  # unnecessary, to be consistent with with function for single state
...
constraint_vals <- input_target$ccoef_sparse %>% 
  group_by(constraint_num) %>% 
  summarise(constraint_val = sum(ccoef / wgt_factor)) 


# constraint_vals_allstates

constraintTargets <- constraint_vals %>% pull(constraint_val)
constraintTargets

input_all <- list(); input_all$ccoef_sparse <- ccoef_sparse
init_vals            <- eval_g_sparse(x0, input_all)

sum(constraintTargets == init_vals)
