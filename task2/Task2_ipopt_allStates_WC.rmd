...
# system.time({outputs <- run_ipopt_allstates(recipe, "xtop")})
# saveRDS(outputs, file = paste0(PROJHOME, "/task2/results/ipopt_allStates.rds"))

outputs <- readRDS(paste0(PROJHOME, "/task2/results/ipopt_allStates.rds"))
...
# Targeted (should be equal to targets by construction)
...
compare_ecdf("intp",  "NY", acs_subset, outputs, 1)
...
compare_corr("pincp", "All", acs_subset, outputs)

compare_corr("pincp", "NY", acs_subset, outputs)
compare_corr("pincp", "CA", acs_subset, outputs)
compare_corr("pincp", "TX", acs_subset, outputs)
compare_corr("pincp", "IL", acs_subset, outputs)
compare_corr("pincp", "FL", acs_subset, outputs)
...
state_names <- as.character(data$stabbr) %>% unique %>% sort
data_stacked %>% select(serialno, sporder, stabbr, stabbr_original, x) %>% 
  #filter(stabbr == "CA") %>% 
  spread(stabbr_original, x) %>% 
  arrange(stabbr) %>% 
  group_by(stabbr) %>% 
  summarise_at(vars(!!state_names), .funs = list(mean =  ~mean(., na.rm = TRUE),
