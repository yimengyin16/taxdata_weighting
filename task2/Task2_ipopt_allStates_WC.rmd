...
recipe <- bind_rows(stateTot_recipe, range_recipe, record_recipe) %>% 
...
```{r}
run_ipopt_allstates_WC <- function(recipe,
                                obj_fn,
                                data = acs_subset){
# Function inputs
#  - target_state
...
system.time({outputs <- run_ipopt_allstates_WC(recipe, "xtop")})
saveRDS(outputs, file = paste0(PROJHOME, "/task2/results/ipopt_allStates_WC.rds"))
