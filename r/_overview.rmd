...
sumnz <- function(var, weight) {(var != 0) * weight * var} # weighted sum of nonzero value
...
### Define generic function to calculate constraint sums based on a given data file and set of weights
```{r}
calc_constraints <- function(weights, ccoef, constraint_vars){
  # calculated weighted sums of contstaint_vars that are in data, uwing the weight variable
  # weights: numeric vector
  # ccoef: df with 1 column per constraint variable (plus possibly additional columns); 1 row per person (tax return), values are constraint coefficients
  # contraint_vars: character vector
  # return: a named vector
  colSums(weights * select(ccoef, constraint_vars))
}
...
filesums <- calc_constraints(pufsub$weight, ccoef_all, names(ccoef_all)) # a named vector

# pick some random x's
set.seed(5)
x <- rnorm(nrow(pufsub), mean=1, sd=.2)
targets <- calc_constraints(pufsub$weight * x, ccoef_all, names(ccoef_all))
...
eval_g_dense <- function(x, inputs){
  ...
  # ipoptr requires that ALL functions receive the same arguments, so the inputs list is passed to ALL functions
  unname(calc_constraints(inputs$weight * x, inputs$data, inputs$constraint_vars)) # ipoptr doesn't seem to like a named vector
}
...
+ Sum of AGI -- E00100_sumpos
+ Sum of wages -- E00200_sumpos
...
constraint_vars <- c("E00100_npos", "E00100_sumpos", "E00200_sumpos", "E02000_nneg")

ccoef <- ccoef_all %>%
  select(constraint_vars)
  
# Get the file sums so we know where we are starting from
(filesums <- calc_constraints(pufsub$weight, ccoef, constraint_vars))
...
# Look at our targets
cbind(init_vals=filesums[names(targets)], targets)
...
calcsums <- calc_constraints(inputs$weights * result1$solution, inputs$data, inputs$constraint_vars)
cbind(filesums, targets, calcsums)
...
# names(result1b) %>% sort
result1b$status; result1b$message; result1b$iterations
result1b$objective; result1$objective
quantile(result1b$solution, probs=c(0, .05, .1, .25, .5, .75, .9, .95, 1))

calcsums <- calc_constraints(inputs$weights * result1b$solution, inputs$data, inputs$constraint_vars)
cbind(filesums, targets, calcsums)
...
+ Creating more complex constraints -- e.g., for subsets of the file
